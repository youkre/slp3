...
第 5 章中介绍的方法（如 word2vec 或 GloVe）为词汇表中的每个唯一词 <span class="math">\(w\)</span> 学习一个固定的向量嵌入。
相比之下，上下文嵌入（例如由 BERT 等掩码语言模型所学习到的表示）则为同一个词 <span class="math">\(w\)</span> 在不同上下文中生成不同的向量表示。
