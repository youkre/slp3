...
回顾第 13.1.2 节，时刻 <span class="math">\(t\)</span> 的隐藏层会参与下一时刻损失的计算，因此对总损失有贡献。
...
它计算前一时刻隐藏状态 <span class="math">\(\mathbf{h}_{t-1}\)</span> 与当前输入 <span class="math">\(\mathbf{x}_t\)</span> 的加权和，并通过 sigmoid 函数生成一个介于 0 到 1 之间的掩码。
随后，该掩码与前一时刻的上下文向量 <span class="math">\(\mathbf{c}_{t-1}\)</span> 进行逐元素相乘，从而清除上下文中不再需要的信息。
两个向量的逐元素相乘（用符号 <span class="math">\(\odot\)</span> 表示，有时也称为**哈达玛积**（Hadamard product））会得到一个与输入向量维度相同的向量，其中第 <span class="math">\(i\)</span> 个元素是两个输入向量第 <span class="math">\(i\)</span> 个元素的乘积：
...
在每个时间步，LSTM 的输出由隐藏状态 <span class="math">\(\mathbf{h}_t\)</span> 提供。
