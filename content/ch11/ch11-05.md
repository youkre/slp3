...
例如，若系统为某问题返回了五个答案，但前三个均错误（即排名最高的正确答案位于第 4 位），则该问题的倒数排名得分为 <span class="math">\(\frac{1}{4}\)</span>。
若系统未返回任何正确答案，则得分为 0。
系统的 MRR 即为所有测试问题得分的平均值。
在某些 MRR 的变体中，得分为 0 的问题会被排除在平均计算之外。
更形式化地，设系统对测试集 <span class="math">\(Q\)</span> 中的每个问题返回一个排序答案列表（或在变体中，令 <span class="math">\(Q\)</span> 表示测试集中得分非零的问题子集），则 MRR 定义为：
