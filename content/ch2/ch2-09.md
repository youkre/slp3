...
# 2.9 最小编辑距离（Minimum Edit Distance）
...
假设某个字符串（例如 `exention`）在该最优路径上（不考虑这个路径是什么样）。
...
我们将采用动态规划自底向上地计算 `D[n,m]`，组合子问题得到答案。
当源中的子字符串长度为 `i`，目标子字符串为空时，从 `i` 个字符到 `0` 个字符需执行 `i` 次删除操作。
若目标子串长度为 `j`，而源子串为空，则从 `0` 个字符变为 `j` 个字符需要 `j` 次插入操作。
在计算出较小 `i` 和 `j` 对应的 `D[i,j]` 后，再基于这些已计算出的较小值，逐步计算更大的 `D[i,j]`。
矩阵中存在三条可能的路径抵达 `D[i,j]`，取其中最小的值：
...
前文提到了两种版本的莱文斯坦距离：一种替换操作代价为 1，另一种替换操作代价为 2（即等价于一次插入加一次删除）。
...
D[i-1,j-1]+
\begin{cases}
2; \quad \text{若} \text{source}[i] \neq \text{target}[j] \\
0; \quad \text{若} \text{source}[i] = \text{target}[j]
\end{cases}
\end{cases}
\tag{2.20}
$$
...
要扩展编辑距离算法以生成对齐结果，可以先将对齐视为编辑距离矩阵中的一条路径。
...
整个计算过程分为两步。
第一步，增强最小编辑距离算法，在每个单元格中存储回溯指针（backpointer）。
回溯指针从当前单元格指向进入该单元格时所来自的前一个（或多个）单元格。
在图 2.21 中展示了这些回溯指针的示意图。
某些单元格有多个回溯指针，因为最小代价的扩展可能来自多个前驱单元格。
第二步是进行**回溯**（backtrace）。
在回溯过程中，从最后一个单元格（位于最后一行和最后一列）开始，沿着指针逆向追踪回动态规划矩阵的起点。
从终点到起点的每一条完整路径都对应一个最小距离的对齐方式。
练习 2.7 要求修改最小编辑距离算法，使其能够存储指针，并执行回溯以输出对齐结果。

<figure class="panel-figure">


![](/slp3/images/slp-fig-2-21.png)


<figcaption class="panel-caption">

**图 2.21** 在向每个单元格填入数值时，用最多三个箭头标记出该值来自相邻的三个单元格中的哪一个。
表格填满后，通过**回溯**（backtrace）计算对齐路径（即最小编辑路径）：从右下角的数字8开始，沿箭头逆向追踪。
加粗单元格的序列代表了两个字符串之间一种可能的最小代价对齐方式，再次使用了插入/删除代价为1、替换代价为2的莱文斯坦距离。
图表设计参考 Gusﬁeld (1997)。

</figcaption>
</figure>

尽管示例使用了简单的莱文斯坦距离，但图 2.19 中的算法允许为各种操作设置任意权重。
