...
因此，这个矩阵的维度是 <span class="math">\(|V| \times |V|\)</span>（<span class="math">\(|V|\)</span> 表示词汇表大小），每个单元格记录的是：在某个训练语料中，某一行的目标词与某一列的上下文词在邻近位置共同出现的次数。
...
但完整的矩阵非常庞大：因为对词汇表中的每个词（共 <span class="math">\(|V|\)</span> 个），都要统计它与其他所有词的共现次数，所以维度是 <span class="math">\(|V| \times |V|\)</span>。
...
例如，*cherry* 被表示为列表 <span class="math">\([1, 0, 1]\)</span>（图 5.2 中的第一行向量），而 *information* 被表示为列表 <span class="math">\([1, 1, 0]\)</span>（第四行向量）。
...
而在真实的词-文档矩阵中，文档向量的维度是 <span class="math">\(|V|\)</span>，即词汇表的大小。
...
我们看看真实数据中的计数是什么样子。
图 5.3 展示了这四个词的部分词-词共现矩阵。由于在教科书页面上无法展示全部 <span class="math">\(|V|\)</span> 个可能的上下文维度，我们从中挑选了 6 个维度，并基于维基百科语料库（Davies, 2015）计算了共现频次。

<figure class="panel-figure">


![](/slp3/images/slp-fig-5-3.png)
...
我们可以把一个文档的向量看作 <span class="math">\(|V|\)</span> 维空间中的一个点。
...
注意：向量的维度 <span class="math">\(|V|\)</span> 通常等于词汇表大小，一般取训练语料中最常见的 10,000 到 50,000 个词。再往后增加低频词通常帮助不大。
