...
在这种设置下，与目标词 <span class="math">\(w\)</span> 余弦相似度最高的词，通常是语义（semantic）相近且词性（part of speech）相同的词。
而当使用长上下文窗口计算向量时，与目标词 <span class="math">\(w\)</span> 余弦相似度最高的词，往往是主题相关但语义不相似的词。
...
早在认知科学的早期研究中，Rumelhart 和 Abrahamson（1973）就提出了一种基于向量空间的**平行四边形模型**（parallelogram model），用于解决形如 <span class="math">\(\textit{a is to b as } a^\ast \textit{ is to what?}\)</span> 的简单类比问题。
例如，给定问题：*apple:tree::grape:?*，即 “*apple 之于 tree，正如 grape 之于 ___*”，系统应填入 *vine*（藤蔓）。
在该模型中（见图 5.8），计算从 *apple* 到 *tree* 的向量偏移：<span class="math">\(\overrightarrow{tree} - \overrightarrow{apple}\)</span>；将此偏移加到 *grape* 的向量上：<span class="math">\(\overrightarrow{grape} + (\overrightarrow{tree} - \overrightarrow{apple})\)</span>；找出距离该结果点最近的词，即为答案。

<figure class="panel-figure">


![](/slp3/images/slp-fig-5-8.png)
...
例如，表达式 <span class="math">\(\overrightarrow{king} - \overrightarrow{man} + \overrightarrow{woman}\)</span> 的结果向量接近 <span class="math">\(\overrightarrow{queen}\)</span>。
同样，<span class="math">\(\overrightarrow{Paris} - \overrightarrow{France} + \overrightarrow{Italy}\)</span> 的结果向量接近 <span class="math">\(\overrightarrow{Rome}\)</span>。
这表明嵌入模型似乎能提取出诸如 MALE–FEMALE（性别）、CAPITAL–CITY-OF（首都-国家）、甚至 COMPARATIVE/SUPERLATIVE（比较级/最高级）等抽象关系（见 GloVe 的图 5.9）。

<figure class="panel-figure">


![](/slp3/images/slp-fig-5-9.png)
...
对于形式为 <span class="math">\(\mathbf{a} : \mathbf{b} :: \mathbf{a}^* : \mathbf{b}^*\)</span> 的问题（即已知向量 <span class="math">\(\mathbf{a}\)</span>、<span class="math">\(\mathbf{b}\)</span>、<span class="math">\(\mathbf{a}^*\)</span>，需找出 <span class="math">\(\mathbf{b}^*\)</span>），平行四边形方法的计算公式为：
...
不过，这一方法存在一些局限。
在 word2vec 或 GloVe 空间中，平行四边形算法返回的最近词通常并非真正的 <span class="math">\(\mathbf{b}^*\)</span>，而是三个输入词之一或其形态变体。
