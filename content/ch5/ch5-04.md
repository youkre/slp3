...
为了度量两个目标词 <span class="math">\(v\)</span> 和 <span class="math">\(w\)</span> 之间的相似性，我们需要一种度量方法，它能接受两个向量（维度相同，要么都以词语为维度，因此长度为 <span class="math">\(|V|\)</span>，要么都以文档为维度，长度为 <span class="math">\(|D|\)</span>），给出它们相似程度的度量值。
...
这种**归一化的点积**实际上等于两个向量之间夹角的余弦，这源于两个向量 <span class="math">\(\mathbf{a}\)</span> 和 <span class="math">\(\mathbf{b}\)</span> 之间点积的定义：
...
因此，两个向量 <span class="math">\(\mathbf{v}\)</span> 和 <span class="math">\(\mathbf{w}\)</span> 之间的**余弦**相似性度量可计算为：
...
因此，可以将向量 <span class="math">\(\mathbf{a}\)</span> 除以 <span class="math">\(|\mathbf{a}|\)</span> 来计算出一个单位向量。
对于单位向量，点积就等于余弦值。
...
例如，要找出与任意目标词 <span class="math">\(w\)</span> 最相似的 10 个词，只需计算 <span class="math">\(w\)</span> 与其余 <span class="math">\(|V| - 1\)</span> 个词的余弦值，排序后取前 10 名即可。